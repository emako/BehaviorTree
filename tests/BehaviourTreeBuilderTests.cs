...
{
    private BehaviorTreeBuilder testObject;
    ...
    [Fact]
    public void cant_create_a_behavior_tree_with_zero_nodes()
    {
        Init();
    ...
    public void cant_create_an_unested_action_node()
    {
        ...
        Assert.Throws<ApplicationException>(() =>
            {
                testObject
                     .Do("some-node-1", t => BehaviorTreeStatus.Running)
                     .Build();
            }
        );
    }
    ...
    public void can_create_inverter_node()
    {
        ...
        Assert.Equal(BehaviorTreeStatus.Failure, node.Tick(new TimeData()));
    }

    [Fact]
    public void cant_create_an_unbalanced_behavior_tree()
    {
        Init();
    ...
    public void can_invert_an_inverter()
    {
        ...
        Assert.IsType<InverterNode>(node);
        Assert.Equal(BehaviorTreeStatus.Success, node.Tick(new TimeData()));
    }
    ...
    public void adding_more_than_a_single_child_to_inverter_throws_exception()
    {
        ...
        Assert.Throws<ApplicationException>(() =>
        {
            testObject
                .Inverter("some-inverter")
                    .Do("some-node", t => BehaviorTreeStatus.Success)
                    .Do("some-other-node", t => BehaviorTreeStatus.Success)
                .End()
                .Build();
        });
    }
    ...
    public void can_create_a_sequence()
    {
        ...
        Assert.Equal(BehaviorTreeStatus.Success, sequence.Tick(new TimeData()));
    ...
    public void can_create_parallel()
    {
        ...
        Assert.Equal(BehaviorTreeStatus.Success, parallel.Tick(new TimeData()));
        Assert.Equal(2, invokeCount);
    }
